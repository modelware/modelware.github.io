...
description: "Part 1 of a series. Rendering a specification as an ontology, with AI reading it and automated reasoning checking it, is not just a way to implement it; it is a way to understand it. We put the OMG Unified Architecture Framework under that neurosymbolic lens, and the first finding, grounded in the spec's own words, is the most telling: UAF was designed to be implemented as an ontology. It says so, and it is built on one."
---
...
At the scale of a real standard, that analysis is not done by logic alone. Reading hundreds of pages of prose, figures, and a profile and proposing how each piece maps into an ontology is fluent, interpretive work, exactly what large language models are good at; deciding whether the result holds together is exacting, unforgiving work, exactly what a reasoner is good at. Pairing the two, AI for fluency and automated reasoning for rigor, is what makes the technique practical at this size, and it has a name in AI research: neurosymbolic analysis.

We applied that instrument to a substantial target: the OMG's Unified Architecture Framework (UAF), the standard that unifies the defense architecture frameworks (DoDAF, MODAF, NAF, DNDAF) into a single grid of viewpoints and aspects, used to plan by capability, trace strategy down to fielded resources, and reason about system-of-systems architectures. We rendered the UAF Domain Metamodel (DMM) as a faithful, platform-neutral ontology on [OML Code](/oml-code), our platform for building and analyzing models, where [AI](/oml-code/capabilities/ai-integration) and [automated reasoning](/oml-code/capabilities/automated-reasoning) work as complementary capabilities, and used it to cross-check every element and relationship against the normative UAF Modeling Language (UAFML) profile.
...
Rendering a specification as an ontology is a form of analysis: the discipline forces its structure, and its seams, into the open. Applied to UAF, the first thing it reveals is that UAF was built for exactly this: IDEAS-ontology-driven down to its Individual/Type/Tuple element legend, and explicitly meant to be implemented beyond UML and SysML.
</aside>

That is why this series is titled *towards* an ontological implementation rather than merely *an* implementation. The findings ahead are best read as what a rigorous lens shows about the distance between what a standard *intends* and how its artifacts are currently *expressed*, a distance ontological analysis is well suited to measuring, and closing.
